...
class AuthService(
    ...
    fun socialLogin(req: SocialLoginRequest): TokenResponse {
        ...
        val socialProfile = when (req.provider.uppercase()) {
            "GOOGLE" -> getGoogleProfile(req.code, req.codeVerifier)
        ...
        // 2. 로그인 또는 회원가입 처리
        // 이메일로 기존 유저 확인. 없으면 새로 생성.
        val user = userRepository.findByEmail(socialProfile.email)
            ?: userRepository.save(
                User(
        ...
        // 3. 우리 서비스 전용 토큰 발급 (JwtTokenProvider 활용)
